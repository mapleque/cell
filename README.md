...
#### 数据迁移
1. user数据，从susuan.wx_edu_student表同步
2. student数据，从suasuan.wx_edu_student表同步
3. product数据，根据现有业务添加
4. resource数据，从susuan.wx_edu_student表同步
5. feature数据，从业务表同步

#### 代码重构
1. 重写登录和token取student逻辑
2. 重写各业务使用资源逻辑
3. 调整支付回调逻辑

#### 测试上线流程
p0 : 功能测试：正常请求相应，异常请求相应，数据库数据部署及同步，日志状态及切分
p1 : 压力测试：响应时间，并发，水平扩展，数据库同步，日志切分
p2 : token线上灰度：接入部分token解析服务，观察服务状态
p3 : token线上全量：接入全部token解析服务，观察服务状态
p4 : 分业务接入用户数据服务
p5 : 分业务接入用户资源服务
